...
model = LightGCN(embed_dim=64, num_layers=3, dataset=yelp.train_data, device=device).to(device)
optimizer = Adam(model.parameters(), lr=0.001)
bpr_loss = BPRLoss()
reg_loss = EmbLoss(1e-5)
print(model)

logger = setup_logger("lightgcn" + ".log", color=False, time=False)
...
def train(epoch, model, data_iter, optimizer, loss_func, device=None):
    ...
    for data in data_iter:
        ...
        users_emb_lightgcn, pos_emb_lightgcn, neg_emb_lightgcn, users_emb, pos_emb, neg_emb = model(users, pos_items, neg_items)
        loss = loss_func(users_emb_lightgcn, pos_emb_lightgcn, neg_emb_lightgcn) + reg_loss(users_emb, pos_emb, neg_emb)
        loss.backward()
        optimizer.step()
        t_loss += loss.item()
...
def test(model, dataset, data_iter, metrics=['recall', 'ndcg', 'mrr'], topks=[10], mode='test', exclude_method=['train'], device=None):
    ...
    if mode == 'test':
        ...
        if 'valid' in exclude_method:
            ...
            train_user_pos_dict = {user: np.concatenate([items, valid_user_pos_dict[user]]) for user, items in train_user_pos_dict.items()}
...
stopping_step = 0
stopping_step_cnt = 50
best_epoch = 0
val_epoch = 1
tmp_model_dir = './saved_model/lightgcn.pkl'
